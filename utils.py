# SCRIPT CON TODAS LAS FUNCIONES DE LOS UTILS QUE NOS HAN DADO
...
def summaryLogReg(modelpipe, X: pd.core.frame.DataFrame, y: pd.core.series.Series):
    ...
    coefs["Signif"] = coefs["Pr(>|t|)"].apply(
        lambda x: (
            "***"
            if x < 0.001
            else (
                "**" if x < 0.01 else ("*" if x < 0.05 else ("." if x < 0.1 else " "))
            )
        )
    )
    print(coefs)
...
def plotModelGridError(
    ...
    if len(param_keys) > 1:
        ...
        multi_index = pd.MultiIndex.from_tuples(
            [(p1, p2) for p1, p2 in sorted(zip(grid1, grid2))], names=[param1, param2]
        )
        dfe = pd.DataFrame(
            [(e, s) for e, s in zip(err, std)], columns=cols, index=multi_index
        )
        # First hyperparameter
        plt.figure(figsize=figsize)
        ax = plt.gca()
        dfe.unstack(level=1)["cv_error"].plot(
            ax=ax, style="o-", yerr=dfe.unstack(level=1)["cv_std"]
        )
        # reset color cycle so that the marker colors match
        ax.set_prop_cycle(None)
        # plot the markers
        sc = dfe.unstack(level=1)["cv_error"].plot(
            figsize=(12, 8), style="o-", markersize=5, ax=ax
        )
        ...
        plt.title(
            f"Best model with {params[0]} = {str(best_params[params[0]])} and {params[1]} = {str(best_params[params[1]])} "
        )
        ...
        plt.figure(figsize=figsize)
        dfe.unstack(level=0)["cv_error"].plot(
            ax=plt.gca(), style="o-", yerr=dfe.unstack(level=1)["cv_std"]
        )
        # reset color cycle so that the marker colors match
        plt.gca().set_prop_cycle(None)
        # plot the markers
        dfe.unstack(level=0)["cv_error"].plot(
            figsize=(12, 8), style="o-", markersize=5, ax=plt.gca()
        )
...
def plot2DClass(
    ...
    if isinstance(X, pd.DataFrame):
        ...
        if any(not elem in c_names for elem in [var1, var2]):
            ...
            err_str = (
                " and ".join(list(itertools.compress([var1, var2], nelems)))
                + " could not be found in X."
            )
    ...
    if selClass is None:
        selClass = y.unique()[0]
        warnings.warn(
            "The first level of the output would be use as positive class",
    ...
    if len(df.columns) == 3:
        ...
        palette = {
            y.unique()[0]: "C0",
            y.unique()[1]: "C1",
        }  # Needed to maintain color palette
        ...
        grid_X1_X2.columns = [var1, var2, output_name] + [
            (
                ...
                else "_".join(["prob", str(np.round(lev, 2))])
            )
            for lev in y.unique()
        ]
        ...
        sns.scatterplot(
            ...
            ax=axes[0, 0],
            data=grid_X1_X2,
        ).set_title("Classification of input space")
        ...
        sns.scatterplot(
            ...
            " ".join(["Probabilities estimated for input space, class:", selClass])
        )

        # Classification results
        df2 = pd.concat(
            [df.reset_index(), interaction(y, df["pred"]).reset_index()], axis=1
        )
        del df2["index"]
        sns.scatterplot(x=var1, y=var2, hue="inter", data=df2, ax=axes[1, 0]).set_title(
            " ".join(["Classification results, class:", selClass])
        )

        df[output_name] = y
        sns.scatterplot(
            x=var1, y=var2, hue=output_name, palette=palette, data=df, ax=axes[1, 1]
        ).set_title(
            " ".join(
                ["Classes and estimated probability contour lines for class:", selClass]
            )
        )
        ...
        plt.subplot(121)
        sns.scatterplot(x=var1, y=var2, hue="inter", data=df2).set_title(
            "Classification of input space"
        )
        plt.subplot(122)
        df[output_name] = y
        sns.scatterplot(x=var1, y=var2, hue=output_name, data=df).set_title(
            " ".join(
                ["Classes and estimated probability contour lines for class:", selClass]
            )
        )
...
def interaction(var1, var2, returntype="Series"):
    ...
    names1 = dVar1.columns[
        np.concatenate(
            repmat(np.arange(len(dVar1.columns)), len(dVar2.columns), 1).transpose()
        )
    ]
    names2 = dVar2.columns[
        np.concatenate(
            repmat(np.arange(len(dVar2.columns)), 1, len(dVar1.columns)).transpose()
        )
    ]
    namesdef = [
        "_".join([str(name1), str(name2)]) for name1, name2 in zip(names1, names2)
    ]
    inter = pd.DataFrame(
        np.multiply(
            dVar1.iloc[
                ...
                np.concatenate(
                    repmat(
                        np.arange(len(dVar1.columns)), len(dVar2.columns), 1
                    ).transpose()
                ),
            ].to_numpy(),
            dVar2.iloc[
                ...
                np.concatenate(
                    repmat(
                        np.arange(len(dVar2.columns)), 1, len(dVar1.columns)
                    ).transpose()
                ),
            ].to_numpy(),
        ),
...
def confusion_matrix(
    ...
    Balanced_acc = mtrs.balanced_accuracy_score(
    ...
    if average == "binary":  # binary
        ...
        out_str = (
            ...
            + "\n"
            + "P-Value [Acc > NIR]: "
            + str(round(res.pvalue, 2))
            + "\n"
            + "Kappa: "
            + str(round(Kappa, 2))
            + "\n"
            + "Mcnemar's Test P-Value: "
            + str(round(MCN_pvalue, 2))
            + "\n"
            + "Sensitivity: "
            + str(round(sens, 2))
            + "\n"
            ...
            + "\n"
            + "Prevalence: "
            + str(round(Prevalence, 2))
            + "\n"
            + "Detection Rate: "
            + str(round(Detection_rate, 2))
            + "\n"
            + "Detection prevalence: "
            + str(round(Detection_prevalence, 2))
            + "\n"
            + "Balanced accuracy: "
            + str(round(Balanced_acc, 2))
            + "\n"
            + "F1 Score: "
            + str(round(F_score, 2))
            + "\n"
    ...
    return acc, sens, spec, precision, F_score
...
def plotClassPerformance(
    output_dir,
    y: pd.core.series.Series,
    prob_est: pd.core.frame.DataFrame,
    selClass: str = None,
    figsize=(10, 5),
    model_name: str = "",
):
    ...
    if len(categories) == 2:
        ...
        try:
            ...
            points_y, points_x = calibration_curve(
                y.values, prob_est, pos_label=selClass, n_bins=10
            )
        ...
        plt.savefig(f"{output_dir}/{model_name}_calibration_plot.png")  # Save plot
        plt.show()
        ...
        g.fig.suptitle(
            "Plot 2/4: Probability of Class " + selClassTitle
        )  # can also get the figure from plt.gcf()
        g.savefig(f"{output_dir}/{model_name}_histogram.png")  # Save plot
        ...
        plt.savefig(f"{output_dir}/{model_name}_roc_curve.png")  # Save plot

        y_true = y == selClass
        accuracy_scores = []
        for thresh in thresholds:
            accuracy_scores.append(
                mtrs.accuracy_score(y_true, [1 if m > thresh else 0 for m in prob_est])
            )
        ...
        plt.savefig(f"{output_dir}/{model_name}_accuracy_plot.png")
    ...
    for col in estimations.columns:
        try:
            ...
            y, x = calibration_curve(
                real,
                estimations.loc[:, col],
                pos_label=real.cat.categories[1],
                n_bins=10,
            )
...
def roc_curve(
    ...
    fig, ax = plt.subplots(figsize=figsize)
    for col in estimations.columns:
        fpr, tpr, thresholds = mtrs.roc_curve(
            real, estimations.loc[:, col], pos_label=selClass
        )
        # only these two lines are calibration curves
